...
const GET_POSTS_BY_TAG = gql`
  query GetPostsByTag($slug: String!) {
    posts(first: 100, where: { tag: $slug, status: PUBLISH, orderby: { field: DATE, order: DESC } }) {
      edges {
        node {
          id
          title
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        ...
        // For tags, we might need to fetch tag name separately, but for now use slug
        setTagName(tagSlug);
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            <nav className="text-sm text-gray-text">
              ...
              <span className="mx-2">/</span>
              <span className="text-gray-500">Tag: {tagName}</span>
            </nav>
          </div>
          ...
          {paginatedPosts.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-2xl border border-gray-border">
              ...
              <p className="text-gray-500 font-medium text-lg">No articles found with this tag.</p>
